...
const Sidebar = {
  _items: [
    ...
    { id: 'settings',   label: 'Pengaturan',        section: null,
  ...
  render() {
    ...
    const compName  = settings.namaPerusahaan || 'BECCA';
    const tagline   = settings.tagline || 'Catering Management';
    const logoUrl   = settings.logoUrl || '';

    let html = `
      <!-- Brand / Company Header -->
      <div class="sidebar-brand" style="padding:16px 14px 12px;border-bottom:1px solid var(--border)">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:0">
          ...
          <div style="width:38px;height:38px;border-radius:8px;overflow:hidden;flex-shrink:0;
            ...
            ${logoUrl
              ...
              : `<div style="font-size:16px;font-weight:800;color:var(--primary-h)">${compName.substring(0,1).toUpperCase()}</div>`}
          </div>
          <!-- Name + Tagline -->
          <div style="min-width:0;flex:1;overflow:hidden">
            ...
            <div style="font-size:10px;color:var(--text-3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${tagline}</div>
          </div>
        </div>
      </div>
    ...
    // Powered by BECCA watermark
    html += `
      <div style="padding:8px 14px 4px;margin-top:auto;border-top:1px solid var(--border)">
        <div style="font-size:9px;color:var(--text-3);text-align:center;opacity:.6;letter-spacing:.04em">
          Powered by <span style="font-weight:700;color:var(--primary-h)">BECCA</span> System
        </div>
      </div>
    `;
