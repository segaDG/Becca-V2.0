/* ============================================
   ...
   Fix v3: sticky background solid + no gap antara # dan Nama
============================================ */
...
const CustomerModule = (() => {
  ...
  /* ── WARNA BARIS — ditentukan di sini, dipakai di td sticky juga ── */
  ...
  function _renderFull(page) {
    ...
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden">
      ...
      <div style="overflow-x:auto;-webkit-overflow-scrolling:touch">
        <style>
          /* ── STICKY: 2 kolom pertama (#=40px, Nama=220px) ── */
          /* FIX: background diset via CSS dengan !important supaya
             tidak kalah dari inline style tr yang diset JS           */
          #cust-table td.s-no,
          #cust-table th.s-no  { position:sticky; left:0;   z-index:3; width:40px; min-width:40px; max-width:40px; }
          #cust-table td.s-nama,
          #cust-table th.s-nama{ position:sticky; left:40px; z-index:3; width:220px; min-width:220px;
                                  border-right:2px solid var(--border2) !important;
                                  box-shadow:3px 0 8px -2px rgba(0,0,0,.3); }
          /* Header row sticky atas */
          #cust-table thead th { position:sticky; top:0; z-index:4; background:var(--primary-h) !important; }
          #cust-table thead tr:first-child th { top:0; z-index:4; background:#1e1e2e !important; }
          #cust-table thead tr:nth-child(2) th { top:28px; z-index:4; }
          #cust-table thead th.s-no,
          #cust-table thead th.s-nama { z-index:6 !important; }

          /* FIX UTAMA: background td sticky harus solid, pakai !important
             agar menang dari apapun yg tr punya (inline style sekalipun) */
          #cust-table tbody tr:nth-child(odd)  td.s-no,
        ...
        <table id="cust-table" style="width:100%;border-collapse:collapse;min-width:2400px;font-size:11px">
          <thead>
            <!-- GROUP HEADER ROW -->
            <tr>
              <!-- # dan Nama digabung karena keduanya sticky, tidak ada kolom lain di antara -->
              <th class="s-no s-nama" colspan="2"
                style="padding:5px 10px;font-size:9px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#9ca3af;background:#1e1e2e !important;border-right:none"></th>
  ...
  function _renderTbody(canEdit) {
    ...
    tbody.innerHTML = list.map((c, i) => {
      ...
      /* FIX: tidak pakai inline style background di tr.
         Background dikendalikan murni via CSS class odd/even + row-hov.
         Hover diset via JS addClass/removeClass supaya sticky td ikut. */
      const sc = ak
      ...
      return `<tr
        ...
        <td class="s-nama" style="padding:8px 12px;font-weight:700;font-size:12px;white-space:nowrap">${c.nama||'-'}</td>
  ...
  function openModal(id) {
    ...
    Modal.open({
      ...
      size: 'modal-xl',
      body: `
        <style>.modal-xl{max-width:900px!important}.cf-section{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:var(--text-3);padding:10px 0 6px;border-top:1px solid var(--border);margin-top:4px}</style>
