/* ============================================
   ...
   Fix: search tidak reset focus, sticky kolom nama saat scroll
============================================ */
...
const CustomerModule = (() => {
  ...
  /* ── INIT — hanya render sekali, lalu partial update ── */
  async function init() {
  ...
  /* ── FULL RENDER — hanya dipanggil pertama kali atau setelah add/edit ── */
    ...
    <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden">
      ...
      <div style="padding:10px 14px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:10px;flex-wrap:wrap">
        <div style="position:relative;flex:1;max-width:280px">
          ...
          <input id="cust-search-input"
            placeholder="Cari nama / jenis / kota..."
            value="${_search}"
      ...
      <!-- TABLE SCROLL — sticky perlu overflow-x:auto di parent langsung tabel -->
      <div style="overflow-x:auto;-webkit-overflow-scrolling:touch">
        <style>
          /* Sticky kolom #1 (No) dan #2 (Nama Perusahaan) */
          #cust-table .col-no  { position:sticky;left:0;z-index:3; }
          #cust-table .col-nama{ position:sticky;left:36px;z-index:3; }
          /* Header sticky juga */
          #cust-table thead th { position:sticky;top:0;z-index:4; }
          #cust-table thead th.col-no  { z-index:6; }
          #cust-table thead th.col-nama { z-index:6; }
          /* Background solid wajib agar tidak transparan saat scroll */
          #cust-table tbody tr:nth-child(odd)  td.col-no,
          #cust-table tbody tr:nth-child(odd)  td.col-nama { background:var(--surface); }
          #cust-table tbody tr:nth-child(even) td.col-no,
          #cust-table tbody tr:nth-child(even) td.col-nama { background:var(--surface2); }
          #cust-table tbody tr:hover td.col-no,
          #cust-table tbody tr:hover td.col-nama { background:rgba(99,102,241,.07); }
          /* Shadow pemisah setelah kolom nama */
          #cust-table td.col-nama, #cust-table th.col-nama {
            box-shadow:3px 0 6px -2px rgba(0,0,0,.25);
        ...
        <table id="cust-table" style="width:100%;border-collapse:collapse;min-width:2400px;font-size:11px">
          <thead>
            <tr style="background:#1e1e2e">
              <th class="col-no"  colspan="1" style="padding:5px 8px;text-align:center;font-size:9px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#9ca3af;background:#1e1e2e"></th>
              <th class="col-nama" colspan="1" style="padding:5px 10px;font-size:9px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#9ca3af;background:#1e1e2e"></th>
            ...
            <tr style="background:var(--primary-h)">
              ${_thFix('col-no','#','center','36px')}
              ${_thFix('col-nama','Nama Perusahaan','left','220px',true)}
  ...
  /* ── PARTIAL RENDER — hanya tbody + counter, input tidak disentuh ── */
  function _renderTbody(canEdit) {
    ...
    // Update counter tanpa menyentuh input
    ...
    tbody.innerHTML = list.map((c, i) => {
      ...
      return `<tr style="border-bottom:1px solid var(--border)"
        ...
        >
        <td class="col-no" style="padding:8px 6px;text-align:center;font-size:10px;color:var(--text-3)">${i+1}</td>
        <td class="col-nama" style="padding:8px 12px;font-weight:700;font-size:12px;white-space:nowrap">${c.nama||'-'}</td>
  ...
  /* ── TH HELPERS ── */
  function _thFix(cls, label, align, width) {
    return `<th class="${cls}" style="padding:8px 10px;text-align:${align||'left'};font-size:10px;font-weight:700;text-transform:uppercase;color:#fff;white-space:nowrap;width:${width};background:var(--primary-h)">${label}</th>`;
  }
  function _thS(field, label, width, align) {
    ...
    return `<th onclick="CustomerModule.sortBy('${field}')"
      style="padding:8px 10px;text-align:${align||'left'};font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:${isActive?'#fbbf24':'#fff'};white-space:nowrap;cursor:pointer;width:${width};background:var(--primary-h)">
      ${label}${arrow}
    </th>`;
  }
  ...
  function setSearch(val) {
    ...
    // FIX BUG 1: Hanya update tbody, TIDAK render ulang seluruh page
    // input tetap fokus karena DOM-nya tidak disentuh
    _renderTbody();
  }
  ...
  function _submit(id) {
    ...
    // Full render setelah save karena stat cards juga perlu update
    _renderFull();
  }
