/* ============================================
   ...
   Real-time alerts: stok menipis, hutang karyawan,
   invoice jatuh tempo, AP jatuh tempo
============================================ */
const NotifCenter = {
  _items: [],
      ...
      employees.filter(e => e.status==='ACTIVE' && (e.sisaHutang||0) > 0)
        ...
        .forEach(e => {
          this._items.push({
            ...
            icon: '💸',
            title: `Hutang: ${e.nama}`,
            desc: `Sisa hutang: ${Utils.formatRupiah(e.sisaHutang)}`,
            action: () => App.navigate('employee'),
          });
        });

      // 3. Invoice jatuh tempo / terlambat
      const invoices = await DB.getInvoices().catch(()=>[]);
      invoices.filter(inv => inv.status !== 'LUNAS' && inv.jatuhTempo)
        .forEach(inv => {
          const isLate = inv.jatuhTempo < today;
          const isSoon = !isLate && inv.jatuhTempo <= soon;
          if (isLate || isSoon) {
            this._items.push({
              type: 'invoice',
              level: isLate ? 'danger' : 'warning',
              icon: '🧾',
              title: isLate ? `Invoice Terlambat: ${inv.noinv||''}` : `Invoice Jatuh Tempo: ${inv.noinv||''}`,
              desc: `${inv.customer||''} · ${Utils.formatDate?Utils.formatDate(inv.jatuhTempo,'dd/mm/yyyy'):inv.jatuhTempo}`,
              action: () => App.navigate('invoice'),
            });
          }
        });

      // 4. AP jatuh tempo / terlambat
      const apList = await DB.getAP().catch(()=>[]);
      apList.filter(ap => ap.status !== 'LUNAS' && ap.jatuhTempo)
        .forEach(ap => {
          const isLate = ap.jatuhTempo < today;
          const isSoon = !isLate && ap.jatuhTempo <= soon;
          if (isLate || isSoon) {
            this._items.push({
              type: 'ap',
              level: isLate ? 'danger' : 'warning',
              icon: '💳',
              title: isLate ? `AP Terlambat: ${ap.supplier||''}` : `AP Jatuh Tempo: ${ap.supplier||''}`,
              desc: `${Utils.formatRupiah(ap.total||0)} · ${Utils.formatDate?Utils.formatDate(ap.jatuhTempo,'dd/mm/yyyy'):ap.jatuhTempo}`,
              action: () => App.navigate('ap'),
            });
          }
        });
          ...
          ${items.map((item, i) => `
            <div onclick="NotifCenter._clickItem(${i})"
                 style="display:flex;align-items:flex-start;gap:var(--s3);
                        ...
                        border-radius:var(--r-md);cursor:pointer;transition:all var(--t-base)"
                 onmouseover="this.style.transform='translateX(4px)'"
                 onmouseout="this.style.transform=''">
              <span style="font-size:20px;flex-shrink:0">${item.icon}</span>
              <div style="flex:1;min-width:0">
                <div style="font-weight:600;font-size:13px;color:${item.level==='danger'?'var(--danger)':'var(--warning)'};margin-bottom:2px">
                  ${item.title}
                </div>
                <div style="font-size:12px;color:var(--text-3)">${item.desc}</div>
              </div>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14" style="flex-shrink:0;color:var(--text-3)">
