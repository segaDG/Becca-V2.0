...
const NotifCenter = {
  ...
  async refresh() {
    ...
    try {
      ...
      const lateTasks = tasks.filter(t=>t.status!=='done'&&t.status!=='arsip'&&t.deadline&&t.deadline<=today);
  ...
  _updateBadge() {
    ...
    if(count>0){
      badge.style.cssText = 'display:flex!important;align-items:center;justify-content:center;position:absolute;top:4px;right:4px;min-width:16px;height:16px;background:var(--danger);border-radius:999px;font-size:8px;font-weight:700;color:white;border:1.5px solid var(--surface);padding:0 3px;';
  ...
  async showPanel() {
    ...
    const mid = Modal.open({
      ...
      size  : 'modal-md',
      body  : items.length===0 ? '<div class="empty-state" style="height:200px"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" width="40" height="40"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg><h4>Semua Aman! 🎉</h4><p>Tidak ada notifikasi saat ini</p></div>' :
        '<div style="display:flex;flex-direction:column;gap:var(--s2)">'+items.map((item,i)=>
          '<div onclick="NotifCenter._clickItem('+i+')" style="display:flex;align-items:flex-start;gap:var(--s3);padding:var(--s3) var(--s4);background:'+(item.level==='danger'?'rgba(239,68,68,.08)':'rgba(245,158,11,.08)')+';border:1px solid '+(item.level==='danger'?'rgba(239,68,68,.2)':'rgba(245,158,11,.2)')+';border-radius:var(--r-md);cursor:pointer;transition:transform .15s" onmouseover="this.style.transform='translateX(4px)'" onmouseout="this.style.transform=''"><span style="font-size:20px;flex-shrink:0">'+item.icon+'</span><div style="flex:1;min-width:0"><div style="font-weight:600;font-size:13px;color:'+(item.level==='danger'?'var(--danger)':'var(--warning)')+';margin-bottom:2px">'+item.title+'</div><div style="font-size:12px;color:var(--text-3)">'+item.desc+'</div></div><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14" style="flex-shrink:0;color:var(--text-3)"><path d="M5 12h14M12 5l7 7-7 7"/></svg></div>'
        ).join('')+'</div>',
